# From https://github.com/TechDudie/tkinterDnD
...
def on_drag_release(event,function=None):
  global grid
  widget = event.widget
  x = round((widget.winfo_x() - widget._drag_start_x + event.x) / 128) * 128
  y = round((widget.winfo_y() - widget._drag_start_y + event.y) / 128) * 128
  widget.place(x=x, y=y)
  if function != None:
    function()
...
def on_component_drag_release(event):
  ...
  widget = event.widget
  container = widget.nametowidget(widget.winfo_parent())
  x = round((container.winfo_x() - container._drag_start_x + event.x) / 128) * 128
  y = round((container.winfo_y() - container._drag_start_y + event.y) / 128) * 128
  container.place(x=x, y=y)
  if function != None:
    function()
